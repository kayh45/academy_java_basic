...
public class ImageCopy {
	...
	public static void main(String[] args) throws IOException {
		...
		FileInputStream in = new FileInputStream("pikachu.png");

		// ==== 입력 node stream 선언, 초기화 끝
		
		// 3. output node stream (FileOutputStream)
		FileOutputStream out = new FileOutputStream("pikachu-copied.png");
		
		// ==== 출력 node stream 선언, 초기화 끝
		
		
		// 2. read 작업 : read() 메소드 사용
		// 노드 스트림의 read() 를 실행할 때 byte[] 을 매개변수로 받는 메소드 사용
		
		// (1) 바이트 배열 : buffer 선언
		byte[] buffer = new byte[2048];
		
		// (2) 노드 스트림의 read() 메소드는 읽어들인 
		//     바이트 수를 정수로 리턴, 읽은 데이터가 없으면 -1 리턴
		int readSize = 0;
		int repeat = 0;
		// (3) read()
